...
public class Plant : MonoBehaviour //TODO: fix NaN health bug
{
...
//        Debug.Log(notEnoughWater);
        ...
        /*if(state == PlantStates.PRODUCE_GAMETES && (notEnoughWater || (energy < seedEnergyCost))){ //max energy depends on biomass by percentage
            SetState(PlantStates.STATIC);
        } else if(state == PlantStates.PRODUCE_GAMETES){
            ProduceMaleGametes();//here produce pollen
        } else if ({
            SetState(PlantStates.PRODUCE_GAMETES);
        }*/
...
//            Debug.Log("Reproduction-----------------");
        ...
        //            Debug.Log("Seed survived!");
                    plantController.InstantiateOffspring(seedPosition, "|---NewPlant_" + offspringCount + "_gen" + (generation + 1), generation + 1,
...
//        Debug.Log(this.biomass + " biomass2");
 ...
 /*   public Vector2 RandomPointInAnnulus(Vector2 origin, float minRadius, float maxRadius){ //to some misc class
