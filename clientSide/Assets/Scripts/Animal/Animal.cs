...
public class Animal : MonoBehaviour
{
    ...
    void Start()
    {
        mover.SetVisionAndSpeed(viewRadius * statsMod.visionMod, speed * statsMod.speedMod); //Set movement and vision to in mover
        ...
        if(isHerbivore){
            adulthoodAge = statsMod.getMaxAge() / 15.0f; //herbivores are more precocial, so grow quicker
            minScale = 0.7f;
        } else {
            adulthoodAge = statsMod.getMaxAge() / 12.0f;
            minScale = 0.6f;
        }
    ...
    float CalculatePreyResistanceMultiplier(float predatorSize, float preySize)
        {
            float x = predatorSize / preySize; // соотношение размеров
            float k = 10f; 
            float midpoint = 0.6f; //хищник получает половину урона здоровью при таком соотношении
...
//            Debug.Log("Offspring on the way");
            ...
            if(female){
                ...
                animalController.InstantiateOffspring
                    ...
                    this.gameObject.GetComponent<DNAstats>(), col.gameObject.GetComponent<DNAstats>(), 
                    this.gameObject.GetComponent<NeatNN>(), col.gameObject.GetComponent<NeatNN>(), isHerbivore); //add network
            } else {
                AlterEnergy(baseStatValue/20, false);
                AlterThirst(baseStatValue/20, false);
            }
    ...
    void ReduceStats(){ 
        ...
        if(mover.getState() != States.REST){
            AlterEnergy(statsMod.energyDrainMod, false);
        }
    }
